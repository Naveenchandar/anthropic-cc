...
import { User, Bot, Loader2, File } from "lucide-react";
...
function getToolLabel(toolName: string, args: Record<string, unknown>): string {
  ...
  if (toolName === "str_replace_editor") {
    switch (command) {
      ...
      case "insert":    return filename ? `Updating ${filename}` : "Updating file";
      case "undo_edit": return filename ? `Reverting ${filename}` : "Reverting changes";
      default:          return filename ? `Working on ${filename}` : "Working on file";
    }
  }
        ...
        {messages.map((message) => (
          ...
          >
            ...
            <div className={cn(
                            ...
                            return (
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    <File className="w-3.5 h-3.5 text-neutral-500 flex-shrink-0" />
              ...
              {message.createdAt && (
                <span className="text-[11px] text-neutral-400 px-1 select-none">
